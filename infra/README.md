...
4. Copy that certificate and private key into ignored `infra/certs/` as `server.crt` and `server.key`; copy the issuer CA as `ca.crt`. The API and Pi trust `ca.crt`; the Mosquitto broker presents `server.crt`.
