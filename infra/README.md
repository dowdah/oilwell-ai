...
6. Set `MQTT_BIND_IP` in the ignored ECS `.env` to the ECS WireGuard/private-overlay address. Compose binds TCP 8883 only to that address; the edge Compose maps `MQTT_HOST` to `MQTT_CONNECT_IP` while retaining TLS hostname validation. Configure ECS security groups to permit TCP 80/443 to the Web audience and explicitly deny public TCP 8883. Do not publish PostgreSQL or API ports.
